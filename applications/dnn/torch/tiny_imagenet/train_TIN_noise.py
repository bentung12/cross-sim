...
# =============================================================================
# This function trains a CrossSim model at a certain TID level
# Important Inputs:
...
def train_model(model, dataloaders, criterion, optimizer, reducer, scheduler, 
    optimizer_name, model_name, TID, alt, dist, mean, std_dev, num_epochs=30, l2_penalty=0):
    ...
    for epoch in range(0,num_epochs):
        ...
        for phase in ['train', 'val']: # changed
            ...
            for layer in analog_modules(model):
                ...
                layer.params = create_params(training_TID)
...
if __name__ == "__main__":
    for i in range(0, 20): #Runs the training 20 times
        ...
        with open(f"outputTinyImageNetNoise{6*n+2}_TestAcc_{TID}.txt", "a") as f:  #Writes the accuracy to a file
            f.write("\nIteration: "+str(i)+"\n")
            f.write('Test accuracy: {:4f}'.format(test_acc))
            
    
        torch.save(cross_sim_model.state_dict(), model_name + "Noise" + "_v2_" + str(optimizer) + "_" + str(num_epochs) + "_" + str(i) + "_" + str(int(TID)) + ".pth") #Saves the model weights
